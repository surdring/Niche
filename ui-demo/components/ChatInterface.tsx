...
import { Message, Role, ScenarioTemplate } from '../types';
...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  const handleSubmit = async () => {
    ...
    const botMsg: Message = {
      ...
      timestamp: Date.now(),
      isThinking: template.capabilities.reasoning // Show thinking indicator if reasoning capable
    };
    ...
    await streamGeminiResponse(
      ...
      (chunk) => {
        setMessages(prev => prev.map(m => {
          if (m.id === botMsgId) {
            return { ...m, content: m.content + chunk, isThinking: false }; // Stop thinking once text flows
          }
          return m;
        }));
      },
      () => {
        setIsLoading(false);
        // Simulate RAG citation injection for demo purposes
