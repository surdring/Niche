---
trigger: manual
purpose: 对 AI-Dev 生成的所有产出物进行严格的、批判性的、无偏见的规范与代码审查
---
...
- **契约一致性**：所有契约必须与 `design-contracts.md` 完全一致
...
- [ ] **文档同步**：契约定义是否与 `design-contracts.md` 一致
...
#### H2. 自动化验证
- [ ] **测试通过**：`npm run test` 是否通过
- [ ] **类型检查**：`npm run typecheck` 是否通过
- [ ] **代码检查**：`npm run lint` 是否通过
- [ ] **构建成功**：`npm run build` 是否成功

#### H3. 验收日志
- [ ] **日志生成**：是否在 `reports/` 目录生成验收日志
- [ ] **日志完整**：日志是否包含任务编号、验证结果、问题清单
- [ ] **日志命名**：日志命名是否符合规范（`YYYY-MM-DD_T{N}_short-slug.md`）
