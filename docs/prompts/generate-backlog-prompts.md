# Backlog 任务详细 Prompt 生成器

## Context
你是 Niche/Study Copilot 项目的技术架构师与任务规划专家。
MVP 阶段（T1-T21）已完成，现在需要为 Backlog 任务（T22-T45）生成详细的执行提示词。

## 当前状态
- MVP 已交付：T1-T21 全部完成
- 工程现状：
  - 已建立 monorepo（packages/core, apps/api, apps/web）
  - 已落地核心契约（GraphQL/REST/Streaming/Events/Citation/Evidence）
  - 已实现 requestId/tenantId/projectId 贯穿与隔离
  - 已接入 RAGFlow 作为外部知识库
  - 已完成 UI 最小闭环（模板选择 → 运行 → 引用 → 导出）
  - 测试体系已建立（单元/集成/契约/E2E）
  - CI/CD 与运维手册已就绪

## 目标
为 Backlog 任务（T22-T45）生成**与 MVP 任务同等详细度**的中文执行提示词。

## 输入
1. 任务范围：指定要生成的任务编号（例如 T22-T27，或 T33-T45）
2. 优先级排序：如有特定优先级需求请说明（例如"优先 T22 多模态输入"）
3. 约束条件：是否允许引入新依赖/新服务/新存储（默认：需明确评估）

## 输出要求

### 1. 遵循 MVP Prompt 的结构模板
...
```
请为 T22-T27 生成详细 Prompt（优先 T24 OCR 支持）。
约束：允许引入新依赖，但需评估成本与维护性。
```

### AI 输出

```markdown
# Backlog Prompts（T22-T27）详细版

## Task T24 - OCR 支持（图片与扫描 PDF）【优先】
[按上述模板生成完整 Prompt]

## Task T22 - 多模态输入（拍题/OCR/语音）
[按上述模板生成完整 Prompt]

## Task T23 - 数据处理管道（清洗/脱敏/增强）
[按上述模板生成完整 Prompt]
...
| 任务 | 新增依赖 | 成本评估 | 风险 |
|------|---------|---------|------|
| T24  | tesseract.js | 低（MIT 协议，社区活跃）| 中（OCR 准确率依赖图片质量）|
| T22  | @google-cloud/vision | 高（按调用量计费）| 低（成熟服务）|
| ...  | ...     | ...     | ...  |

## 执行顺序建议
1. T23（数据处理管道）- 为 T24 提供下游消费能力
2. T24（OCR 支持）- 优先级最高
3. T22（多模态输入）- 依赖 T24
4. T25-T27（架构扩展）- 可并行
```
...
- [ ] Execution Plan 至少 3 个步骤，每步骤有 Scope/Files/Requirements
- [ ] Verification 至少 3 条可自动化断言
- [ ] Checklist 至少 5 条可验收交付物
- [ ] Output 明确具体产物（代码/文档/测试）
- [ ] 明确依赖关系与前置条件
- [ ] 明确是否引入新依赖/新服务
- [ ] 明确高风险点与回滚策略
- [ ] 错误分支至少覆盖 2 个场景
- [ ] 与现有契约/模块的对接点明确
- [ ] 可观测性要求明确（日志/metrics/requestId）

## 注意事项

1. **保持一致性**：所有 Backlog Prompt 的详细度必须与 MVP Prompt（T1-T21）一致
2. **避免过度设计**：技术选型给出建议即可，不强制（例如"优先 Tesseract.js，也可选 Google Vision API"）
3. **强调可替换性**：涉及外部服务的必须抽象 Adapter 层
4. **契约优先**：所有输入/输出必须有 Zod Schema
5. **自动化优先**：Verification 必须可自动化，避免"手工目测"
...
1. 你要为哪些任务生成详细 Prompt？（例如 T22-T27，或 T33-T45）
2. 是否有特定优先级？
3. 是否允许引入新依赖/新服务？
