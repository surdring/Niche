...
#### T33-T45 - Research Copilot 扩展（R28-R40）
- 关联需求：R28-R40
- 依赖：T10, T11, T12
- 产出/变更点：
  - PDF 结构化解析/页码引用、联网搜索入库、研究工作台、Notion 导出、强引用模式、RAGFlow 契约约束、长文档策略、深度解析、坐标级引用、混合检索、按页深读、本地模型与异步解析、研究项目 bootstrap。
- Done criteria：
  - 每个子能力均具备独立的端到端演示与回归用例。
