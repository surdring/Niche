# Study Copilot - Requirements

## 1. 背景与目标
本需求文档用于将 `study-copilot-driven-requirements.md` 中“按落地路径重排”的需求，抽取为便于追踪与实现的 `R#` 编号体系。

- 本迭代优先目标：让 Study Copilot 的“模板可运行、可控、可观测、可交互”的主干闭环跑通。
- 约束：
  - RAGFlow 仅作为外部知识库服务（摄入/检索/引用溯源），回答生成仍由系统通过 Vercel AI SDK 调用模型完成。
  - GraphQL 为主要业务接口；REST/流式接口用于上传、事件流与高吞吐场景。
  - 当前迭代不实现多租户与配额管理能力（但要预埋 tenantId/projectId）。
...
> 说明：本文将原始需求文档中的每条“需求 xx”逐条转为 `R#`，并保留“来源：需求 xx”用于追溯。
...
#### R31 - 导出到 Markdown/Obsidian 和 Notion
- 来源：需求 27
