...
interface TemplateSelectorProps {
  templates?: ScenarioTemplate[]; // 可选：从后端加载的模板
  isLoading?: boolean;
...
export const TemplateSelector: React.FC<TemplateSelectorProps> = ({ 
  ...
  // 使用传入的模板或默认模板
  const displayTemplates = templates && templates.length > 0 ? templates : SCENARIO_TEMPLATES;
      ...
      {isLoading ? (
        <div className="flex items-center justify-center py-20">
          <div className="animate-pulse flex flex-col items-center gap-4">
            <Icons.Cpu className="w-12 h-12 text-accent animate-spin" />
            <p className="text-slate-400">加载场景模板...</p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full">
          ...
          <button
            ...
            className="group relative flex flex-col items-start p-6 bg-secondary border border-slate-700 rounded-xl hover:border-accent hover:shadow-xl hover:shadow-accent/10 transition-all duration-300 text-left hover:-translate-y-1"
          >
            ...
            <div className="w-full border-t border-slate-700 pt-4 mt-auto">
              <div className="flex flex-wrap gap-2">
                ...
                {template.capabilities.coding && (
                  <span className="text-xs px-2 py-1 bg-orange-900/30 text-orange-300 rounded border border-orange-900/50">代码辅助</span>
                )}
              </div>
            </div>
