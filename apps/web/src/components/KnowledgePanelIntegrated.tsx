...
interface KnowledgePanelIntegratedProps {
  citations: Citation[];
  isVisible: boolean;
  tenantId: string;
  projectId: string;
...
export const KnowledgePanelIntegrated: React.FC<KnowledgePanelIntegratedProps> = ({ 
  citations, 
  isVisible,
  tenantId,
  projectId 
  ...
  const handleCitationClick = async (citationId: string) => {
    ...
    try {
      ...
      const result = await fetchEvidence({ tenantId, projectId, requestId }, citationId);
      setEvidence(result);
    } catch (error) {
            ...
            {citations.length === 0 ? (
              ...
              citations.map((cite) => (
                <div 
                  ...
                  className={`bg-slate-800 rounded-lg p-3 border transition-colors cursor-pointer group ${
                    selectedCitationId === cite.id 
                      ? 'border-accent' 
                ...
                >
                  <div className="flex items-start justify-between mb-1">
                    ...
                    <Icons.ArrowRight className={`w-3 h-3 transition-colors ${
                      selectedCitationId === cite.id ? 'text-accent' : 'text-slate-600 group-hover:text-accent'
                    }`} />
                  </div>
          ...
          <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-700">
            ...
            {isLoadingEvidence ? (
              ...
              <div className="text-xs text-red-400 py-2">
                ...
                <p className="text-slate-500">{evidenceError}</p>
              </div>
            ) : evidence ? (
              <div className="space-y-2 text-xs">
                <div>
                  <span className="text-slate-500">来源ID:</span>
                  <span className="text-slate-300 ml-2 font-mono">{evidence.sourceId}</span>
                </div>
                {evidence.snippet && (
                  <div>
                    <span className="text-slate-500">摘录:</span>
                    <p className="text-slate-300 mt-1 p-2 bg-slate-900/50 rounded text-xs leading-relaxed">
                      {evidence.snippet}
                    </p>
                  </div>
                )}
                {evidence.locator && (
                  <div>
                    ...
                    <div className="text-slate-300 ml-2 space-y-1">
                      {evidence.locator.page && <div>页码: {evidence.locator.page}</div>}
                      {evidence.locator.section && <div>章节: {evidence.locator.section}</div>}
                    </div>
                  </div>
                )}
              </div>
