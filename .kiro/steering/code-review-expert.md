...
### 4. 审查报告生成规范
- **文件命名规范**：
  - 格式：`reports/YYYY-MM-DD_T{N}_short-slug.md`
...
#### H2. 自动化验证（基于验收日志）
- [ ] **测试通过**：验收日志是否记录 `npm run test` 通过
- [ ] **类型检查**：验收日志是否记录 `npm run typecheck` 通过
- [ ] **代码检查**：验收日志是否记录 `npm run lint` 通过
- [ ] **构建成功**：验收日志是否记录 `npm run build` 成功
